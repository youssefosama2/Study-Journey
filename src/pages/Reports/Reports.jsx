...
export default function Reports() {
    ...
    async (silent = false) => {
      try {
        ...
        setData({
          ...
          subjectUnits:
            subjectUnitsResult.data || [],
          subjectLessons:
            subjectLessonsResult.data || [],
        });
      } catch (err) {
        console.error(
          "Reports load error:",
        ...
        setError(
          err?.message ||
            "حدث خطأ أثناء تحميل التقارير من قاعدة البيانات"
        );
      } finally {
        setLoading(false);
        setRefreshing(false);
  ...
  const subjectLessonMap = useMemo(
    () =>
      new Map(
        data.subjectLessons.map((lesson) => [
          lesson.id,
          lesson,
        ])
      ),
  ...
  const resolveUnit = useCallback(
    (row) => {
      ...
      if (
        ...
        unitMap.has(row.curriculum_unit_id)
      ) {
        return unitMap.get(
          row.curriculum_unit_id
        );
      ...
      if (
        row.unit_id &&
        unitMap.has(row.unit_id)
      ) {
        return unitMap.get(row.unit_id);
      }

      // Legacy
      if (
        row.unit_id &&
        subjectUnitMap.has(row.unit_id)
      ) {
        return subjectUnitMap.get(
          row.unit_id
        );
      }
  ...
  const resolveLesson = useCallback(
    (row) => {
      ...
      if (
        ...
        lessonMap.has(row.curriculum_lesson_id)
      ) {
      ...
      if (
        row.lesson_id &&
        subjectLessonMap.has(row.lesson_id)
      ) {
        return subjectLessonMap.get(
          row.lesson_id
        );
      }
  ...
  const resolveSubjectId = useCallback(
    (row) => {
       ...
       * unit_id
       *
       * في الـ Curriculum الجديد:
       * units.subject_id
       *
       * وفي القديم:
       * subject_units.subject_id
       */
      if (row.unit_id) {
        const unit =
          unitMap.get(row.unit_id) ||
          subjectUnitMap.get(row.unit_id);
      ...
      if (row.lesson_id) {
        const lesson =
          ...
          subjectLessonMap.get(row.lesson_id);
  ...
  /*
   * =========================================================
   * Helpers الخاصة بالدروس
   * =========================================================
   */

  const getStudyLessonId = useCallback(
    (row) => {
      if (!row) return null;
  ...
  const getReviewLessonId = useCallback(
    (row) => {
      if (!row) return null;
  ...
  const subjectStats = useMemo(() => {
    ...
    periodData.studies.forEach(
      (row) => {
        ...
        const lessonId =
          getStudyLessonId(row);

        if (lessonId) {
          stat.studiedLessons.add(
            lessonId
          );
        }
    ...
    periodData.focusSessions.forEach(
      (row) => {
        const subjectId =
          resolveSubjectId(row);
    ...
    periodData.lessonReviews.forEach(
      (row) => {
        const subjectId =
          resolveSubjectId(row);
    ...
    periodData.reviews.forEach(
      (row) => {
        ...
        const reviewLessonId =
          getReviewLessonId(row);

        /*
         * لو فيه detailed review
         * لنفس الدرس لا نكررها.
         */
        const hasDetailedReview =
          reviewLessonId &&
          periodData.lessonReviews.some(
            (detail) =>
              getReviewLessonId(
                detail
              ) ===
      ...
      .map((subject) => {
        ...
        const reviewCompletion =
          subject.reviewsTotal > 0
            ? (subject.reviewsCompleted /
                subject.reviewsTotal) *
              100
            : 0;

        const errorCorrection =
          subject.errorsTotal > 0
            ? (subject.errorsCorrected /
                subject.errorsTotal) *
              100
            : 0;
  ...
  const overview = useMemo(() => {
    ...
    const focusSeconds =
      periodData.focusSessions.reduce(
        (sum, row) =>
          sum +
          safeNumber(
            row.actual_seconds
    ...
    /*
     * نستخدم detailed reviews
     * أولاً لو موجودة.
     */
    const reviewsTotal =
      periodData.lessonReviews
        .length ||
    ...
    const errorsTotal =
      periodData.errors.length;

    const correctedErrors =
      periodData.errors.filter(
        (error) =>
          error.is_corrected ||
          error.status ===
            "corrected"
  ...
  const trendData = useMemo(() => {
    ...
    for (
      ...
      const focus =
        periodData.focusSessions.filter(
          (row) => {
            const rowDate =
              getDateFromRow(
                row,
    ...
    () =>
      subjectStats
        ...
        .map((subject) => ({
          ...
          tasks:
            Number(
              subject.taskCompletion.toFixed(
                1
              )
            ),
          ...
          reviews:
            Number(
              subject.reviewCompletion.toFixed(
                1
              )
            ),
        })),
    ...
    useMemo(() => {
      ...
      if (
        ...
        insights.push({
          ...
          icon: <FiAward />,
          title:
            "مستواك الدراسي ممتاز",
          text: `متوسط نتائجك ${Math.round(
            overview.averageScore
          )}% خلال ${
            periodInfo.label
          }. حافظ على نفس المستوى.`,
        });
      } else if (
        overview.averageScore >=
        70
      ...
      const rows = [
        ...
        ...subjectStats.map(
          (subject) => [
            ...
            subject.studyHours.toFixed(
              1
            ),
            subject.focusHours.toFixed(
              1
            ),
            Math.round(
              subject.taskCompletion
            ),
            Math.round(
              subject.reviewCompletion
            ),
            Math.round(
              subject.errorCorrection
            ),
          ]
        ),
      ];
        ...
        <section className="reports-kpis">
          ...
          <div className="report-kpi-card">
            ...
            <div>
              ...
              <small>
                {overview.studyDays} أيام مذاكرة
              </small>
            </div>
          </div>
