...
const FocusStatistics = () => {
  ...
  /*
   * sessions:
   * الجلسات الخاصة بالفترة المختارة.
   */
  const [sessions, setSessions] = useState([]);

  /*
   * allFocusSessions:
   * كل جلسات التركيز المكتملة/الموجودة للمستخدم.
   *
   * نستخدمها خصوصًا لحساب الـ streak بشكل صحيح
   * بغض النظر عن الفترة المختارة.
   */
    ...
    async ({ silent = false } = {}) => {
      try {
        ...
        if (!user) {
          ...
          setSubjectNames({});
          setError(
            "يجب تسجيل الدخول لعرض إحصائيات التركيز"
          );
          return;
        }
        ...
        /* =================================================
           ...
           نحتاجها لحساب الـ streak الحقيقي.
        ================================================= */
        ...
        const periodSessions = periodData || [];
        ...
        if (subjectIds.length === 0) {
          ...
          if (subjectsError) {
            console.error(
              "FocusStatistics subjects error:",
              subjectsError
            );
          } else {
            const namesMap = {};
        ...
        console.error(
          "FocusStatistics fetch error:",
          err
        );

        setError(
          err?.message ||
            "حدث خطأ أثناء تحميل إحصائيات التركيز"
        );
  ...
  const goalStats = useMemo(() => {
    ...
    const yes = evaluated.filter(
      (session) =>
        session.goal_result === "yes"
    ).length;

    const partial = evaluated.filter(
      (session) =>
        session.goal_result === "partial"
    ).length;

    const no = evaluated.filter(
      (session) =>
        session.goal_result === "no"
    ).length;
  ...
  /* =====================================================
     ...
     مهم:
     الـ streak يتحسب من كل جلسات التركيز
     وليس من sessions الخاصة بالفترة المختارة.
  ===================================================== */
  ...
  const streak = useMemo(() => {
    ...
    for (
      ...
      if (hasFocus) {
        ...
        /*
         * لو مفيش جلسة النهارده
         * نكمل عشان نحسب آخر streak
         * بدأ من أمس.
         */
        if (i === 0) {
          continue;
      ...
      <div className="focus-statistics-container">
        ...
        {loading ? (
          <div className="focus-statistics-loading">
            ...
            <strong>
              جاري تحميل إحصائيات التركيز...
            </strong>

            <span>
              لحظات ونجهز لك تقريرك
            </span>
          </div>
        ) : (
          <>
            ...
            <section className="focus-statistics-box focus-statistics-distribution-box">
              <div className="focus-statistics-box-header">
                <div>
                  <h2>
                    توزيع وقت التركيز على المواد
                  </h2>

                  <span>
                    كيف وزعت وقت مذاكرتك؟
                  </span>
                </div>
                          ...
                          return (
                            ...
                            >
                              <circle
                                ...
                                className={`focus-statistics-donut-segment segment-${
                                  index %
                                  5
                                }`}
                      ...
                      (
                        ...
                        >
                          <div className="focus-statistics-distribution-name">
                            <span
                              className={`focus-statistics-distribution-dot segment-${
                                index %
                                5
                              }`}
                            />
            ...
            <section className="focus-statistics-box recent-sessions-box">
              <div className="focus-statistics-box-header">
                <div>
                  ...
                  <span>
                    أحدث جلساتك خلال الفترة المحددة
                  </span>
                </div>
